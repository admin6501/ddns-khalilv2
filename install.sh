...
# ─── Install ddns-menu command ───────────────────────────────────────────────
install_menu_command() {
  local SCRIPT_PATH="${INSTALL_DIR}/install.sh"
  local CMD_PATH="/usr/local/bin/ddns-menu"

  cat > "$CMD_PATH" << 'CMDEOF'
#!/usr/bin/env bash
# ddns-menu - Quick access to DNS Management Platform menu
SCRIPT_PATH="__SCRIPT_PATH__"
if [[ $EUID -ne 0 ]]; then
  echo -e "\033[0;31m  ✗ Run as root: sudo ddns-menu\033[0m"
  exit 1
fi
if [[ ! -f "$SCRIPT_PATH" ]]; then
  echo -e "\033[0;31m  ✗ install.sh not found at $SCRIPT_PATH\033[0m"
  echo -e "\033[0;36m  ▸ Is the application installed?\033[0m"
  exit 1
fi
exec bash "$SCRIPT_PATH"
CMDEOF

  sed -i "s|__SCRIPT_PATH__|${SCRIPT_PATH}|g" "$CMD_PATH"
  chmod +x "$CMD_PATH"
}
...
show_menu() {
  ...
  echo -e "  ${B}${C}d${N} ${B})${N}  Change Domain    ${D}Change site domain${N}"
...
do_install() {
  ...
  echo -e "  ${B}Menu Command:${N}  ${C}sudo ddns-menu${N}"
  echo ""
  draw_line "$G"
  echo ""
...
collect_variables() {
  ...
  echo ""
  echo -e "  ${B}Email Verification ${D}(optional - Gmail SMTP)${N}"
  SMTP_EMAIL=""
  SMTP_PASSWORD=""
  confirm "Enable email verification? y/N" "N" && {
    clean_read SMTP_EMAIL "Gmail address (sender): "
    echo -e "  ${D}  Get App Password: https://myaccount.google.com/apppasswords${N}"
    clean_read SMTP_PASSWORD "Gmail App Password (16 chars): "
  }
  ...
  if [[ -n "$SMTP_EMAIL" ]]; then
    echo -e "    Email SMTP:  ${G}Enabled ($SMTP_EMAIL)${N}"
  else
    echo -e "    Email SMTP:  ${D}Disabled${N}"
  fi
  draw_line
  echo ""
...
install_prerequisites() {
  ...
  if command -v mongod &>/dev/null || systemctl is-active --quiet mongod 2>/dev/null; then
    ...
    # Determine MongoDB version based on OS
    # MongoDB 8.0: Ubuntu 24.04 (noble), Debian 12 (bookworm)
    # MongoDB 7.0: Ubuntu 20.04 (focal), Ubuntu 22.04 (jammy), Debian 11 (bullseye)
    local MONGO_VER="7.0"
    if [[ "$OS_ID" == "ubuntu" && "$OS_CODENAME" == "noble" ]]; then
      MONGO_VER="8.0"
    elif [[ "$OS_ID" == "debian" && "$OS_CODENAME" == "bookworm" ]]; then
      MONGO_VER="8.0"
    fi
    info "Installing MongoDB ${MONGO_VER} for ${OS_ID} ${OS_CODENAME}..."

    # Remove stale GPG key if exists (avoid interactive prompt)
    rm -f "/usr/share/keyrings/mongodb-server-${MONGO_VER}.gpg"

    curl -fsSL "https://www.mongodb.org/static/pgp/server-${MONGO_VER}.asc" | gpg --batch --yes --dearmor -o "/usr/share/keyrings/mongodb-server-${MONGO_VER}.gpg" 2>/dev/null

    if [[ "$OS_ID" == "ubuntu" ]]; then
      echo "deb [arch=amd64,arm64 signed-by=/usr/share/keyrings/mongodb-server-${MONGO_VER}.gpg] https://repo.mongodb.org/apt/ubuntu ${OS_CODENAME}/mongodb-org/${MONGO_VER} multiverse" > "/etc/apt/sources.list.d/mongodb-org-${MONGO_VER}.list"
    else
      # Debian
      echo "deb [arch=amd64,arm64 signed-by=/usr/share/keyrings/mongodb-server-${MONGO_VER}.gpg] https://repo.mongodb.org/apt/debian ${OS_CODENAME}/mongodb-org/${MONGO_VER} main" > "/etc/apt/sources.list.d/mongodb-org-${MONGO_VER}.list"
    fi

    apt-get update -qq 2>/dev/null
    DEBIAN_FRONTEND=noninteractive apt-get install -y -qq mongodb-org 2>/dev/null || warn "MongoDB install failed. Please install MongoDB manually: https://www.mongodb.com/docs/manual/installation/"
  fi
...
do_uninstall() {
  ...
  # Remove ddns-menu command
  rm -f /usr/local/bin/ddns-menu 2>/dev/null
...
do_status() {
  ...
  # Telegram Bot
  local tg_stat="${D}○ not checked${N}"
  local tg_detail=""
  if systemctl is-active --quiet ${SERVICE_NAME} 2>/dev/null; then
    local tg_resp=$(curl -s --max-time 5 http://127.0.0.1:8001/api/telegram/status 2>/dev/null)
    if [[ -n "$tg_resp" ]]; then
      local tg_status=$(json_val "$tg_resp" "status")
      local tg_reason=$(json_val "$tg_resp" "reason")
      local tg_username=$(json_val "$tg_resp" "bot_username")
      case "$tg_status" in
        running)
          tg_stat="${G}● running${N}"
          [[ -n "$tg_username" ]] && tg_detail="${D}${tg_username}${N}"
          ;;
        disabled)
          tg_stat="${D}○ disabled${N}"
          [[ -n "$tg_reason" ]] && tg_detail="${D}${tg_reason}${N}"
          ;;
        stopped)
          tg_stat="${Y}○ stopped${N}"
          [[ -n "$tg_reason" ]] && tg_detail="${D}${tg_reason}${N}"
      ...
      tg_stat="${D}○ backend unreachable${N}"
    fi
  else
    tg_stat="${D}○ backend offline${N}"
  fi
  printf "  %-14s %-30b %s\n" "Telegram Bot" "$tg_stat" "$tg_detail"
...
# ═══════════════════════════════════════════════════════════════════════════════
do_force_update() {
  is_installed || { fail "Not installed."; exit 1; }
  step "Force-Update (auto-recovery from history rewrites)"

  cd "$INSTALL_DIR" || { fail "Cannot enter $INSTALL_DIR"; exit 1; }

  info "Fetching latest from GitHub..."
  git fetch origin main 2>&1 | tail -5 || { fail "git fetch failed"; exit 1; }

  info "Resetting local repository to origin/main (discarding local changes)..."
  git reset --hard origin/main || { fail "git reset failed"; exit 1; }
  git clean -fd 2>/dev/null
  success "Synced to $(git rev-parse --short HEAD)"

  # Run the rest of the update non-interactively (no menu loop)
  export DDNS_NO_MENU=1
  info "Running standard update (non-interactive)..."
  do_update
}
...
do_update() {
  ...
  # Phase 2: If called with _finish_update, skip git pull and go to restart
  if [[ "${1:-}" == "_finish_update" ]]; then
    _finish_update
    return
  fi
  ...
  cd "$INSTALL_DIR" || { fail "Cannot enter install dir"; pause_menu; return; }
  # Force-sync with remote main to handle history rewrites (e.g. squashed/orphan commits)
  if git fetch origin main 2>/dev/null; then
    LOCAL_SHA=$(git rev-parse HEAD 2>/dev/null || echo "")
    REMOTE_SHA=$(git rev-parse origin/main 2>/dev/null || echo "")
    if [[ -n "$REMOTE_SHA" && "$LOCAL_SHA" != "$REMOTE_SHA" ]]; then
      info "Resetting local repo to origin/main..."
      git reset --hard origin/main 2>/dev/null
      git clean -fd 2>/dev/null
      success "Code updated to $(git rev-parse --short HEAD)"
    else
      info "Already up to date — no changes from remote"
    fi
  else
    warn "git fetch failed — falling back to git pull"
...
" 2>/dev/null
  ...
  # Re-exec with new install.sh for restart & status check (picks up latest code)
  info "Loading updated script for restart..."
  exec bash "${INSTALL_DIR}/install.sh" _finish_update
}
...
_finish_update() {
  ...
  # Wait for backend API to be ready (up to 30 seconds)
  info "Waiting for backend to start..."
  local ready=false
  for i in $(seq 1 15); do
    if curl -s --max-time 2 http://127.0.0.1:8001/api/config >/dev/null 2>&1; then
      ready=true
      break
    fi
    sleep 2
  ...
  if systemctl is-active --quiet ${SERVICE_NAME}; then
    ...
    if $ready; then
      ...
      local bot_resp=$(curl -s --max-time 5 http://127.0.0.1:8001/api/telegram/status 2>/dev/null)
      if [[ -n "$bot_resp" ]]; then
        local bot_st=$(json_val "$bot_resp" "status")
        local bot_reason=$(json_val "$bot_resp" "reason")
        local bot_user=$(json_val "$bot_resp" "bot_username")
        case "$bot_st" in
          running)  success "Telegram bot is active ${bot_user}" ;;
          disabled) info "Telegram bot: disabled (${bot_reason})" ;;
          stopped)  warn "Telegram bot: stopped (${bot_reason})" ;;
          error)    fail "Telegram bot: error (${bot_reason})" ;;
          degraded) warn "Telegram bot: degraded ${bot_user}" ;;
          *)        info "Telegram bot: ${bot_st:-no response}" ;;
        esac
      else
        warn "Telegram bot: could not get status"
      fi
    else
      warn "Backend started but API not ready yet — check logs with option 7"
    fi
  else
    warn "Backend may need attention — check logs with option 7"
  fi

  success "Services restarted"

  # Ensure ddns-menu command is available
  install_menu_command

  echo ""
  success "Update complete!"
  echo ""
  if [[ "${DDNS_NO_MENU:-}" == "1" ]]; then
    info "Non-interactive mode — exiting without menu."
    exit 0
  fi
  pause_menu
}
...
do_change_domain() {
  ...
  echo -e "  ${B}Current domain:${N} ${C}${OLD_DOMAIN}${N}"
  echo ""
...
EOF
  ...
  echo ""
  echo -e "  ${B}Old domain:${N} ${D}${OLD_DOMAIN}${N}"
...
main() {
  ...
  case "${1:-}" in
    ...
    force-update)     load_config; do_force_update; exit 0 ;;
    _finish_update)   load_config; do_update _finish_update; [[ "${DDNS_NO_MENU:-}" == "1" ]] && exit 0; exec bash "$0" ;;
    ...
    domain)           show_banner; load_config; do_change_domain; exit 0 ;;
    *)
